...
class MultiStage:
    ...
    def create_multi_stage_brackets(self) -> dict:
        ...
        if self.stage_info.get("group_type") == "SE":
            for _ in range(len(self.participants) // self.stage_info.get("compete_in_group")):
                if counter == self.time_managment.get("groups_per_day"):
                    counter = 0
                    self.time_managment["start_time"] = self.time_managment["start_time"] + datetime.timedelta(
                        seconds=86400
                    )
            ...
            for _ in range(len(self.participants) // self.stage_info.get("compete_in_group")):
                ...
                group_stage = RoundRobin(
                    self.participants[start:end], {"win": 1, "loss": 0, "draw": 0}, self.time_managment
                )
        ...
        TBO_participants = [
            ...
            for i in range(
                ...
                // self.stage_info.get("compete_in_group")
                * self.stage_info.get("advance_from_group")
            )
        ]
        ...
        if self.time_managment["final_stage_time"]:
            ...
            self.time_managment["start_time"] = (
                self.time_managment["start_time"]
                - datetime.timedelta(seconds=self.time_managment["start_time"].timestamp())
                + datetime.timedelta(seconds=final_time.timestamp())
                + datetime.timedelta(minutes=self.time_managment.get("break_between"))
                + datetime.timedelta(
                    minutes=self.time_managment.get("avg_game_time") * self.time_managment.get("max_games_number")
                )
            )
    ...
    def set_match_score(match: dict, instance) -> None:
        ...
        if instance.type == "RR" or instance.type == "SW":
            ...
            if stage_played:
                if instance.type == "RR":
                    table = sorted(
                        instance.bracket.get("table"),
                        reverse=True,
                        key=lambda partic: (partic.get("scores"), partic.get("berger")),
                    )
                else:
                    table = sorted(
                        instance.bracket.get("table"),
                        reverse=True,
                        key=lambda partic: (partic.get("scores"), partic.get("buchholz")),
                    )
...
class RoundRobin:
    ...
    def set_match_score(match: dict, bracket) -> None:
        ...
        first_partic = next(
            partic
            for partic in bracket.get("table")
            if partic["participant"] == match.get("participants")[0].get("participant")
        )
        second_partic = next(
            partic
            for partic in bracket.get("table")
            if partic["participant"] == match.get("participants")[1].get("participant")
        )
        ...
        # P -> S
            ...
            if prev_match_state.get("participants")[0]["isWinner"]:
                # add res in table
                first_partic["win"] -= 1
                second_partic["loose"] -= 1
            elif prev_match_state.get("participants")[1]["isWinner"]:
                # add res in table
                second_partic["win"] -= 1
                first_partic["loose"] -= 1
            else:
        ...
        # P -> P
            ...
            # d -> 1
            if (
                not prev_match_state.get("participants")[0]["isWinner"]
                and not prev_match_state.get("participants")[1]["isWinner"]
                and match.get("participants")[0]["isWinner"]
            ):
                first_partic["win"] += 1
                first_partic["draw"] -= 1
                second_partic["draw"] -= 1
            ...
            # d -> 2
                ...
                not prev_match_state.get("participants")[0]["isWinner"]
                and not prev_match_state.get("participants")[1]["isWinner"]
                and match.get("participants")[1]["isWinner"]
            ):
                second_partic["win"] += 1
                second_partic["draw"] -= 1
                first_partic["draw"] -= 1
            ...
            # 2 -> 1
                ...
                match.get("participants")[0]["isWinner"]
                and not prev_match_state.get("participants")[0]["isWinner"]
            ):
                first_partic["win"] += 1
                first_partic["loose"] -= 1
                second_partic["loose"] += 1
            ...
            # 1 -> 2
                ...
                match.get("participants")[1]["isWinner"]
                and not prev_match_state.get("participants")[1]["isWinner"]
            ):
                second_partic["win"] += 1
                second_partic["loose"] -= 1
            ...
            # 1 -> d
                ...
                not match.get("participants")[0]["isWinner"]
                and not match.get("participants")[1]["isWinner"]
                and prev_match_state.get("participants")[0]["isWinner"]
            ):
                first_partic["win"] -= 1
                first_partic["draw"] += 1
                second_partic["draw"] += 1
            ...
            # 2 -> d
                ...
                not match.get("participants")[0]["isWinner"]
                and not match.get("participants")[1]["isWinner"]
                and prev_match_state.get("participants")[1]["isWinner"]
            ):
                second_partic["win"] -= 1
                second_partic["draw"] += 1
                first_partic["draw"] += 1
        ...
        first_partic["scores"] = (
            first_partic["draw"] * draw_scoore + first_partic["win"] * win_scoore + first_partic["loose"] * loos_scoore
        )
        second_partic["scores"] = (
            second_partic["draw"] * draw_scoore
            + second_partic["win"] * win_scoore
            + second_partic["loose"] * loos_scoore
        )
        ...
        for count, i in enumerate(bracket.get("rounds")[0 : round_id + 1]):
            for j in i:
                ...
                first = next(
                    partic
                    for partic in bracket.get("table")
                    if partic["participant"] == j.get("participants")[0].get("participant")
                )
                second = next(
                    partic
                    for partic in bracket.get("table")
                    if partic["participant"] == j.get("participants")[1].get("participant")
                )
                if count == 0:
                    first["berger"] = 0
                    second["berger"] = 0
                if j.get("participants")[0]["isWinner"]:
                    first["berger"] += second["scores"]
                elif j.get("participants")[1]["isWinner"]:
                    second["berger"] += first["scores"]
                elif (
                    match.get("state") == "PLAYED"
                    and not j.get("participants")[0]["isWinner"]
                    and not j.get("participants")[1]["isWinner"]
                ):
                    first["berger"] += 1 / 2 * second["scores"]
                    second["berger"] += 1 / 2 * first["scores"]

    def append_participant(self, name: str) -> dict:
        return {"id": secrets.token_hex(16), "score": 0, "participant": f"{name}", "isWinner": False}
    ...
    def create_round_robin_bracket(self) -> dict:
        ...
        if self.time_managment.get("time_managment"):
            ...
            for counter, round in enumerate(round_robin_bracket):
                if counter == 0:
                    for i in range(
                        self.time_managment.get("mathes_same_time"),
                        len(round),
                        self.time_managment.get("mathes_same_time"),
                    ):
                        for game in round[i : i + self.time_managment.get("mathes_same_time")]:
                            game["startTime"] = (
                                f"{last_time + datetime.timedelta(minutes=self.time_managment.get('avg_game_time') * self.time_managment.get('max_games_number'))}"
                            )
                        last_time = last_time + datetime.timedelta(
                            minutes=self.time_managment.get("avg_game_time")
                            * self.time_managment.get("max_games_number")
                        )
                else:
...
class SingleEl:
    ...
    def append_participant(self, name: str) -> dict:
        return {"id": secrets.token_hex(16), "participant": f"{name}", "score": 0, "isWinner": False}
    ...
    def get_match(self, full: bool = True) -> dict:
        ...
        return {
            ...
            "startTime": f"{self.time_managment.get('start_time')}",
            "state": "SCHEDULED",
            "teams": [self.get_participant(), second_participant],
        }
        ...
        for j in participants[: final.participants_from_group]:
            for i in range(len(final.bracket[0]["seeds"]) // 2):
                if final.bracket[0]["seeds"][i].get("teams")[0].get("participant") == "TBO":
                    ...
                    final.bracket[0]["seeds"][len(final.bracket[0]["seeds"]) // 2 + i]
                    .get("teams")[0]
                    .get("participant")
                    == "TBO"
                ):
                    final.bracket[0]["seeds"][len(final.bracket[0]["seeds"]) // 2 + i]["teams"][0]["participant"] = (
                        j.get("participant")
                    )
                    break
                    ...
                    final.bracket[0]["seeds"][len(final.bracket[0]["seeds"]) // 2 + i]["teams"][1]["participant"] = (
                        j.get("participant")
                    )
                    break
    ...
    def set_match_score(current_match, bracket) -> None:
        ...
        # P -> P
            ...
            current_winner = (
                1 if int(current_match["teams"][0]["score"]) < int(current_match["teams"][1]["score"]) else 0
            )
            ...
            if current_winner != prev_winner:
                ...
                if bracket[round_id] != bracket[-1] and len(bracket[round_id]["seeds"]) != len(bracket[-1]["seeds"]):
                    ...
                    bracket[round_id + 1]["seeds"][match_index]["teams"][team_index]["participant"] = current_match[
                        "teams"
                    ][current_winner]["participant"]
                    bracket[round_id + 1]["seeds"][match_index]["teams"][team_index]["id"] = current_match["teams"][
                        current_winner
                    ]["id"]

                    # check for second final
                    if bracket[round_id] != bracket[-2] and len(bracket[round_id + 1]["seeds"]) == len(
                        bracket[round_id + 2]["seeds"]
                    ):
                        bracket[round_id + 2]["seeds"][match_index]["teams"][team_index]["participant"] = current_match[
                            "teams"
                        ][current_winner - 1]["participant"]
                        bracket[round_id + 2]["seeds"][match_index]["teams"][team_index]["id"] = current_match["teams"][
                            current_winner - 1
                        ]["id"]
        # S -> P
            ...
            if bracket[round_id] != bracket[-1] and len(bracket[round_id]["seeds"]) != len(bracket[-1]["seeds"]):
                ...
                bracket[round_id + 1]["seeds"][match_index]["teams"][team_index]["id"] = current_match["teams"][winner][
                    "id"
                ]
    ...
    def create_se_bracket(self) -> list:
        ...
        # O(n)
        for _ in range(int(number_of_match)):
            if full_first > 0:
        ...
        if nummber_of_rounds > 1:
            ...
            for i in range(1, nummber_of_rounds):
                ...
                # O(n)
                for _ in range(int(number_of_match / 2**i)):
...
class DoubleEl:
        ...
        for j in participants[: final.participants_from_group]:
            for i in range(len(final.bracket["upper_rounds"][0]["seeds"]) // 2):
                if final.bracket["upper_rounds"][0]["seeds"][i].get("teams")[0].get("participant") == "TBO":
                    ...
                    final.bracket["upper_rounds"][0]["seeds"][len(final.bracket["upper_rounds"][0]["seeds"]) // 2 + i]
                    .get("teams")[0]
                    .get("participant")
                    == "TBO"
                ):
                    final.bracket["upper_rounds"][0]["seeds"][len(final.bracket["upper_rounds"][0]["seeds"]) // 2 + i][
                        "teams"
                    ][0]["participant"] = j.get("participant")
                    break
                    ...
                    final.bracket["upper_rounds"][0]["seeds"][len(final.bracket["upper_rounds"][0]["seeds"]) // 2 + i][
                        "teams"
                    ][1]["participant"] = j.get("participant")
                    break
                ...
                if prev_match.get("id") == current_match.get("id"):
                    ...
                    if prev_match.get("state") == "PLAYED" and current_match.get("state") == "SCHEDULED":
                        ...
                        for i in range(r_index, len(bracket["upper_rounds"]) - 1):
                            ...
                            bracket["upper_rounds"][i + 1]["seeds"][cur_i]["teams"][team_index]["id"] = (
                        ...
                        bracket["lower_rounds"][round_index_lower]["seeds"][match_index_lower]["teams"][
                            team_index_lower
                        ]["score"] = 0
                        bracket["lower_rounds"][round_index_lower]["seeds"][match_index_lower]["teams"][
                            team_index_lower
                        ...
                        for i in range(round_index_lower, len(bracket["lower_rounds"]) - 1):
                            ...
                            bracket["lower_rounds"][i + 1]["seeds"][cur_i]["teams"][team_index]["id"] = (
                                secrets.token_hex(16)
                            )

                        # check last
                        lower_winner = (
                            0 if int(prev_match["teams"][0]["score"]) < int(prev_match["teams"][1]["score"]) else 1
                        )

                        if (
                            bracket["upper_rounds"][-1]["seeds"][-1]["teams"][1]["id"]
                            == current_match["teams"][lower_winner]["id"]
                        ):
                    ...
                    # P -> P
                        ...
                        current_winner = (
                            1
                            if int(current_match["teams"][0]["score"]) < int(current_match["teams"][1]["score"])
                            else 0
                        )
                        prev_winner = (
                            1 if int(prev_match["teams"][0]["score"]) < int(prev_match["teams"][1]["score"]) else 0
                        )
                        if current_winner != prev_winner:
                            # rollback
                            cur_i = m_index
                            ...
                            if r_index > 1:
                                ...
                                round_index_lower = r_index
                            bracket["lower_rounds"][round_index_lower]["seeds"][match_index_lower]["state"] = (
                                "SCHEDULED"
                            )
                            bracket["lower_rounds"][round_index_lower]["seeds"][match_index_lower]["teams"][
                                team_index_lower
                            ...
                            # forward
                            if bracket["upper_rounds"][r_index] != bracket["upper_rounds"][-1] and len(
                                bracket["upper_rounds"][r_index]["seeds"]
                            ) != len(bracket["upper_rounds"][-1]["seeds"]):
                                if m_index % 2 == 0:
                                ...
                                bracket["upper_rounds"][r_index + 1]["seeds"][match_index]["teams"][team_index][
                                    "participant"
                                ] = current_match["teams"][current_winner]["participant"]
                                bracket["upper_rounds"][r_index + 1]["seeds"][match_index]["teams"][team_index][
                                    "id"
                                ] = current_match["teams"][current_winner]["id"]
                            bracket["lower_rounds"][round_index_lower]["seeds"][match_index_lower]["teams"][
                                team_index_lower
                            ]["participant"] = current_match["teams"][current_winner - 1]["participant"]
                            bracket["lower_rounds"][round_index_lower]["seeds"][match_index_lower]["teams"][
                                team_index_lower
                            ]["id"] = current_match["teams"][current_winner - 1]["id"]
                    ...
                    # S -> P
                        ...
                        if bracket["upper_rounds"][r_index] != bracket["upper_rounds"][-1]:
                            ...
                            bracket["upper_rounds"][r_index + 1]["seeds"][match_index]["teams"][team_index][
                                "participant"
                            ] = current_match["teams"][winner]["participant"]
                            bracket["upper_rounds"][r_index + 1]["seeds"][match_index]["teams"][team_index]["id"] = (
                                current_match["teams"][winner]["id"]
                            )
                            # indexs for lower bracket
                            if r_index == 0:
                                if m_index % 2 == 0:
                            ...
                            bracket["lower_rounds"][round_index_lower]["seeds"][match_index_lower]["teams"][
                                team_index_lower
                            ]["participant"] = current_match["teams"][winner - 1]["participant"]
                            bracket["lower_rounds"][round_index_lower]["seeds"][match_index_lower]["teams"][
                                team_index_lower
                            ]["id"] = current_match["teams"][winner - 1]["id"]
                    # leave function
                    return
        # lower
                    ...
                    # P -> P
                        ...
                        if current_winner != prev_winner:
                            ...
                            if bracket["lower_rounds"][r_index] != bracket["lower_rounds"][-1]:
                                ...
                                bracket["lower_rounds"][r_index + 1]["seeds"][match_index]["teams"][team_index][
                                    "participant"
                                ] = current_match["teams"][current_winner]["participant"]
                                bracket["lower_rounds"][r_index + 1]["seeds"][match_index]["teams"][team_index][
                                    "id"
                                ...
                                bracket["upper_rounds"][-1]["seeds"][-1]["teams"][1]["participant"] = current_match[
                                    "teams"
                                ][current_winner]["participant"]
                                bracket["upper_rounds"][-1]["seeds"][-1]["teams"][1]["id"] = current_match["teams"][
                                    current_winner
                                ]["id"]
                    # S -> P
                    elif prev_match.get("state") == "SCHEDULED" and current_match.get("state") == "PLAYED":
                        ...
                        if bracket["lower_rounds"][r_index] != bracket["lower_rounds"][-1]:
                            ...
                            bracket["lower_rounds"][r_index + 1]["seeds"][match_index]["teams"][team_index]["id"] = (
                                current_match["teams"][winner]["id"]
                            )
                        else:
                            bracket["upper_rounds"][-1]["seeds"][-1]["teams"][1]["participant"] = current_match[
                            ...
                            bracket["upper_rounds"][-1]["seeds"][-1]["teams"][1]["id"] = current_match["teams"][winner][
                                "id"
                            ]
                    # leave function
                    return
    ...
    def create_de_bracket(self) -> list:
        ...
        for i in range(2, nummber_of_rounds + 1):
            ...
            # O(n)
            for _ in range(int(2**nummber_of_rounds / 2**i)):
        ...
        if self.time_managment.get("time_managment"):
            ...
            for round in lower_rounds:
                ...
                last_time = last_time + datetime.timedelta(minutes=self.time_managment.get("break_between"))
            upper_rounds[-1]["seeds"][0]["startTime"] = (
                f"{last_time + datetime.timedelta(minutes=self.time_managment.get('avg_game_time') * self.time_managment.get('max_games_number'))}"
            )
...
class Swiss:
        ...
        for j in participants[: final.participants_from_group]:
            for i in range(len(final.bracket["rounds"][0]) // 2):
                if final.bracket["rounds"][0][i].get("participants")[0].get("participant") == "TBO":
                    ...
                    final.bracket["rounds"][0][len(final.bracket["rounds"][0]) // 2 + i]
                    .get("participants")[0]
                    .get("participant")
                    == "TBO"
                ):
                    final.bracket["rounds"][0][len(final.bracket["rounds"][0]) // 2 + i]["participants"][0][
                        "participant"
                    ] = j.get("participant")
                    print(3)
                    break
                    ...
                    final.bracket["rounds"][0][len(final.bracket["rounds"][0]) // 2 + i]["participants"][1][
                        "participant"
                    ] = j.get("participant")
                    print(4)
                    break
    ...
    def set_match_score(match: dict, bracket: list) -> None:
        ...
        if (
            round_end
            and round_id != len(bracket.get("rounds")) - 1
            and bracket.get("rounds")[round_id + 1][0].get("participants")[0].get("participant") == "TBO"
        ):
                ...
                for index in range(1, len(parts)):
                    if parts[index].get("participant") not in previously_played:
                        t1 = {
                            "id": parts[0].get("id"),
                            "score": 0,
                            "participant": parts[0].get("participant"),
                            "isWinner": False,
                        }
                        parts.pop(0)
                        t2 = {
                            "id": parts[index - 1].get("id"),
                            "score": 0,
                            "participant": parts[index - 1].get("participant"),
                            "isWinner": False,
                        }
                        parts.pop(index - 1)
                        round.append(
                    ...
                    round.append(
                        {
                            ...
                            "startTime": f"{self.time_managment.get('start_time')}",
                            "state": "SCHEDULED",
                            "participants": [self.append_participant("TBO"), self.append_participant("TBO")],
                        }
                    )
