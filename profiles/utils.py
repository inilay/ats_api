...
def send_email_for_verify(user):
    ...
    send_mail(
        "Verify email", "", "ilya.savelev.2001@gmail.com", [user.email], html_message=message, fail_silently=False
    )
...
def send_email_for_reset(user):
    ...
    send_mail(
        "Change password", "", "ilya.savelev.2001@gmail.com", [user.email], html_message=message, fail_silently=False
    )
